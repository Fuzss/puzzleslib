...
public abstract class FabricDataAttachmentBuilder<T extends AttachmentTarget, V> extends DataAttachmentBuilder<T, V> {

    @Override
    public DataAttachmentType<T, V> build(ResourceLocation id) {
        AttachmentType<V> attachmentType = AttachmentRegistry.create(id, (AttachmentRegistry.Builder<V> builder) -> {
            builder.initializer(() -> {
