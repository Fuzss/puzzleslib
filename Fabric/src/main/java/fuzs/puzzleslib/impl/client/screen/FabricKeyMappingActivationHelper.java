...
public final class FabricKeyMappingActivationHelper implements KeyMappingActivationHelper {
    private static final Multimap<KeyActivationContext, KeyMapping> KEY_MAPPINGS_BY_ACTIVATION = Multimaps.newListMultimap(Maps.newEnumMap(KeyMappingActivationHelper.KeyActivationContext.class), Lists::newArrayList);
    ...
    private void clearKeys(Map<KeyMapping, InputConstants.Key> keys, Predicate<KeyMapping> filter, Supplier<InputConstants.Key> key) {
        ...
        if (!(screen instanceof KeyBindsScreen)) {
            KeyMappingActivationHelper.KeyActivationContext inactive = screen != null ? KeyMappingActivationHelper.KeyActivationContext.GAME : KeyMappingActivationHelper.KeyActivationContext.SCREEN;
