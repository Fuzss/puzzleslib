...
public class ForgeConfigHolderImpl implements ConfigHolder.Builder {
     ...
     */
    private Map<Class<? extends AbstractConfig>, ForgeConfigDataHolderImpl<? extends AbstractConfig>> configsByClass = Maps.newIdentityHashMap();

    @SuppressWarnings("unchecked")
    @Override
    public <T extends AbstractConfig> ConfigDataHolder<T> getHolder(Class<T> clazz) {
        ForgeConfigDataHolderImpl<? extends AbstractConfig> holder = this.configsByClass.get(clazz);
    ...
    public void bakeConfigs(String modId) {
        ...
        for (ForgeConfigDataHolderImpl<? extends AbstractConfig> holder : this.configsByClass.values()) {
