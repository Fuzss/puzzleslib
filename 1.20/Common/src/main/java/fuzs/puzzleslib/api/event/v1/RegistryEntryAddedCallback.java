...
public interface RegistryEntryAddedCallback<T> {
    ...
    static <T> EventInvoker<RegistryEntryAddedCallback<T>> registryEntryAdded(ResourceKey<? extends Registry<? super T>> resourceKey) {
     ...
     */
    void onRegistryEntryAdded(ResourceLocation id, T entry, BiConsumer<ResourceLocation, Supplier<T>> registrar);
}
