...
public final class BlockStateResolverContextNeoForgeImpl implements BlockStateResolverContext {
    ...
    private final Map<ResourceLocation, BlockModel> unbakedPlainModels;
    ...
    public void registerBlockStateResolver(Block block, Consumer<BiConsumer<BlockState, UnbakedModel>> blockStateConsumer) {
        ModelDiscovery discovery = new ModelDiscovery(this.modelResolver, this.missingModel);
        ...
        discovery.discoverDependencies();
        this.loadModels(unbakedBlockStateModels).blockStateModels().forEach(this.blockStateModelOutput);
    }

    private ModelBakery.BakingResult loadModels(Map<ModelResourceLocation, UnbakedBlockStateModel> unbakedBlockStateModels) {
     ...
     */
    private static ModelBakery.BakingResult loadModels(ProfilerFiller profiler, Function<Material, TextureAtlasSprite> textureGetter, ModelBakery modelBakery, Supplier<TextureAtlasSprite> missingSprite) {
        ...
        ModelBakery.BakingResult bakingResult = modelBakery.bakeModels(new ModelBakery.TextureGetter() {
            @Override
            public TextureAtlasSprite get(ModelResourceLocation modelLocation, Material material) {
                return null;
            }
