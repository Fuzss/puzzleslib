...
public final class AbstractLootProviderV2 {
    ...
    public abstract static class EntityTypes extends EntityLootSubProvider implements LootTableDataProvider {
        ...
        @Override
        protected final void add(EntityType<?> entityType, ResourceLocation lootTableLocation, LootTable.Builder builder) {
            this.map.computeIfAbsent(entityType, (EntityType<?> entityTypeX) -> new HashMap<>())
                    .put(lootTableLocation, builder);
        }
    ...
    public interface LootTableDataProvider extends DataProvider {
        ...
        default CompletableFuture<?> run(CachedOutput output) {
            ...
            return CompletableFuture.allOf(lootTables.entrySet().stream().map((entry) -> {
                ...
                return DataProvider.saveStable(output, LootDataType.TABLE.parser().toJsonTree(lootTable), path);
            }).toArray(CompletableFuture[]::new));
        }

        default void validate(Map<ResourceLocation, LootTable> tables) {
            ValidationContext validationContext = new ValidationContext(LootContextParamSets.ALL_PARAMS,
