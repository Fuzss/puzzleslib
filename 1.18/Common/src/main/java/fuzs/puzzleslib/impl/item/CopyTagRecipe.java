...
public interface CopyTagRecipe {
    ...
    static RecipeSerializer<?> getModSerializer(String modId, String recipeSerializerId) {
        RecipeSerializer<?> recipeSerializer = Registry.RECIPE_SERIALIZER.get(new ResourceLocation(modId, recipeSerializerId));
        Objects.requireNonNull(recipeSerializer, "%s serializer for %s is null".formatted(recipeSerializerId, modId));
        return recipeSerializer;
    }

    static void registerSerializers(BiConsumer<String, Supplier<RecipeSerializer<?>>> registrar, Factory factory) {
    ...
    interface Factory {
        ...
        <T extends CraftingRecipe, S extends CraftingRecipe & CopyTagRecipe> RecipeSerializer<S> apply(RecipeSerializer<T> serializer, BiFunction<T, Ingredient, S> factory);
    }

    interface Serializer<T extends CraftingRecipe, S extends CraftingRecipe & CopyTagRecipe> extends RecipeSerializer<S> {

        RecipeSerializer<T> serializer();

        BiFunction<T, Ingredient, S> factory();

        @Override
        default S fromJson(ResourceLocation recipeId, JsonObject serializedRecipe) {
            T recipe = this.serializer().fromJson(recipeId, serializedRecipe);
            Ingredient ingredient = Ingredient.fromJson(GsonHelper.getAsJsonObject(serializedRecipe, "copy_from"));
            return this.factory().apply(recipe, ingredient);
        }

        @Override
        default S fromNetwork(ResourceLocation recipeId, FriendlyByteBuf buffer) {
        ...
        @Override
        default void toNetwork(FriendlyByteBuf buffer, S recipe) {
