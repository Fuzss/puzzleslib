...
/**
 * Copied from Forge Config Api Port to allow for using the exact same command as on Fabric in-game.
 * <p>
...
public class NeoForgeConfigCommand {
        ...
        dispatcher.register(LiteralArgumentBuilder.<P>literal("config")
                .then(RequiredArgumentBuilder.<P, String>argument("mod", new ModIdArgument() {
                            ...
                            public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
                                return SharedSuggestionProvider.suggest(ModList.get()
                                        .applyForEachModContainer(ModContainer::getModId)
                                        .filter(NeoForgeConfigCommand::anyModConfigsExist), builder);
                            }
                        })
                        .then(((RequiredArgumentBuilder<P, ?>) (RequiredArgumentBuilder<?, ?>) RequiredArgumentBuilder.argument(
                                "type",
                                enumConstant(Type.class)
                        )).executes(commandContext -> NeoForgeConfigCommand.showFile(component -> feedbackSender.accept(
                                        commandContext.getSource(),
                                        component
                                ),
                                commandContext.getArgument("mod", String.class),
                                commandContext.getArgument("type", Type.class).unwrap()
                        )))));
    }
    ...
    private enum Type implements StringRepresentable {
        ...
        public ModConfig.Type unwrap() {
            return ModConfig.Type.valueOf(this.name());
        }
