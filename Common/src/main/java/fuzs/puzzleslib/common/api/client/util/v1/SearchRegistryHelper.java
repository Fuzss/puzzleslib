package fuzs.puzzleslib.common.api.client.util.v1;
...
public final class SearchRegistryHelper {
    private static final Map<ContextKey<?>, Entry<?>> SEARCH_TREES = new IdentityHashMap<>();
     ...
     */
    public static <T> void register(ContextKey<T> type, Function<List<T>, SearchTree<T>> factory) {
     ...
     * @param <T>    search item type
     */
    public static <T> void populateSearchTree(ContextKey<T> type, List<T> values) {
     ...
     */
    public static <T> SearchTree<T> getSearchTree(ContextKey<T> type) {
        return lookupEntry(type).searchTree.join();
    }

    private static <T> Entry<T> lookupEntry(ContextKey<T> type) {
        Entry<?> entry = SEARCH_TREES.get(type);
        Objects.requireNonNull(entry, () -> "Search tree type " + type.name() + " is not registered");
