...
public record CompostableBlocksContextNeoForgeImpl(String modId,
                                                   Map<Holder<Item>, Compostable> compostingChances) implements CompostableBlocksContext {
    ...
    @Override
    public void registerCompostable(float compostingChance, ItemLike... items) {
    ...
    private void registerCompostable(ItemLike item, float compostingChance) {
            ...
            DataProviderHelper.registerDataProviders(this.modId, (NeoForgeDataProviderContext context) -> {
                return new DataMapProvider(context.getPackOutput(), context.getRegistries()) {
                    ...
                    protected void gather(HolderLookup.Provider registries) {
                        ...
                        for (Map.Entry<Holder<Item>, Compostable> entry : CompostableBlocksContextNeoForgeImpl.this.compostingChances()
                                .entrySet()) {
        ...
        this.compostingChances.put(item.asItem().builtInRegistryHolder(), new Compostable(compostingChance));
    }
}
